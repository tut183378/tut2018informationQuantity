...
public class Frequencer implements FrequencerInterface{
	...
	// ****  Please write code here... ***
	//
	int saj = 0, sajp = 0, temp = 0;
	
	for(int i = space.length - 1; i > 0; i--){
	    for(int j = 0; j < i; j++){
		saj = suffixArray[j];
		sajp = suffixArray[j+1];
		while(1 > 0) {
		    //配列外参照を阻止
		    if(saj > space.length - 1) break;
		    //スペース内の文字を比較し、suffixArrayの要素をソートする
		    if(mySpace[saj] > mySpace[sajp]){
			temp = suffixArray[j+1];
			suffixArray[j+1] = suffixArray[j];
			suffixArray[j] = temp;
			break;
		    }else if(mySpace[saj] < mySpace[sajp]) break;
		    
		    saj++;
		    sajp++;

		    if(sajp == space.length){
			temp = suffixArray[j+1];
			suffixArray[j+1] = suffixArray[j];
			suffixArray[j] = temp;
			break;
	...
	// ****  Please write code here... ***
	//
	int i = subByteStartIndex(start,end);
	int j = 0;
