...
public class Frequencer implements FrequencerInterface{
	...
	//
		int suffix_i = suffixArray[i]; 
		int target_start_end = end - j;
		int min;
		if(mySpace.length-suffix_i < target_start_end) min = mySpace.length - suffix_i;
		else min = target_start_end;
		for(int	k = 0; k < min; k++){
	    	if(mySpace[suffix_i + k] > myTarget[j + k]) return 1;
	    	if(mySpace[suffix_i + k] < myTarget[j + k]) return -1;
		}
		if(target_start_end > mySpace.length - suffix_i) return -1;
		return 0; // This line should be modified.
    }
